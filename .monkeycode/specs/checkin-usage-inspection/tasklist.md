...
- [ ] 1. 数据模型与迁移（设计：数据模型 / 使用记录）
  - [ ] 1.1 新增迁移 `0018_device_usage_records.sql`
...
- [ ] 3. 考勤化签到与看板（需求 2、3）
  - [ ] 3.1 拆分 `domain/attendance.rs` 的 `check_in`
...
- [ ] 4. 签到内嵌环境与设备检查（需求 5）
  - [ ] 4.1 `check_in` 请求支持 `inspection_items` 与 `is_abnormal`
...
- [ ] 5. 设备使用记录接口（需求 6）
  - [ ] 5.1 实现 `GET /api/usage` 查询
    - 支持 `device_id`、`class_id`、`student_keyword`、`start_date`、`end_date` 过滤（需求 6.5）
    - 返回使用人、开始/结束时间、时长与检查结果摘要（需求 6.6）
  - [ ] 5.2 实现 `POST /api/usage/:id/end` 手动结束会话（需求 6.4）
  - [ ] 5.3 实现 `GET /api/usage/export` 导出（需求 6.7）
  - [ ] 5.4 在 `api/mod.rs` 注册使用记录路由
  - [ ]* 5.5 编写使用记录接口单元测试
    - 覆盖过滤条件、手动结束、导出字段、空结果（需求 6.8）

- [ ] 6. 检查点 - 确保后端编译与测试通过,如有疑问请询问用户

- [ ] 7. 设备端强制签到准入（需求 3、4、5）
  - [ ] 7.1 `campus-checkin` 窗口改为全屏准入
    - 设置为全屏、无边框、置顶（需求 4.1）
    - 新增 `keyhook.rs` 并屏蔽 Windows 键、Alt+Tab、Alt+F4、Ctrl+Esc、任务管理器与全屏切换快捷键（需求 4.2）
  - [ ] 7.2 `campus-checkin` 分阶段流程
  ...
  - [ ] 7.4 `agent-core/src/mode.rs` 与 `attendance.rs` 拉起并监控准入
...
- [ ] 8. 锁屏界面中文化与美化（需求 7）
  - [ ] 8.1 `campus-lock/src/main.rs` 文案改为简体中文（需求 7.1）
  - [ ] 8.2 布局分区优化
...
- [ ] 9. 教师端考勤管理与看板（需求 2）
  - [ ] 9.1 扩展 `api/attendance.ts` 与 `api/types.ts`
    - 新增过滤参数、考勤看板与导出接口（需求 2.2、2.6）
  - [ ] 9.2 改造 `views/Attendance.vue`
...
- [ ] 10. 教师端设备使用记录模块（需求 6）
  - [ ] 10.1 新增 `api/usage.ts` 与类型（需求 6.5、6.7）
  - [ ] 10.2 新增 `views/Usage.vue`
    - 筛选区（设备、班级、学生、日期范围）（需求 6.5）
    - 列表展示使用人、开始/结束时间、时长与检查结果摘要（需求 6.6）
    - 手动结束会话与导出（需求 6.4、6.7）
    - 空状态提示（需求 6.8）
  - [ ] 10.3 在 `router/index.ts` 与 `components/Layout.vue` 注册「设备使用」菜单与路由

- [ ] 11. 教师端授课模式切换班级选择（需求 1）
  - [ ] 11.1 扩展 `api/devices.ts` 切换接口支持 `classId`
  - [ ] 11.2 改造 `views/Devices.vue` 模式切换弹窗
    - 授课模式要求选择班级后方可提交（需求 1.1）
    - 展示进入授课模式与进入锁定模式的设备数量（需求 1.6）
    - 切换被拒绝时展示原因并保持原模式（需求 1.7）

- [ ] 12. 检查点 - 确保前端构建与前后端联调通过,如有疑问请询问用户

- [ ] 13. 打包与产物同步
  - [ ] 13.1 构建前端并同步到 `teacher-server/static/`
  - [ ] 13.2 交叉编译 Windows teacher-server 与 student-agent
  - [ ] 13.3 更新 `dist/windows-release/` 并重建发布 zip
