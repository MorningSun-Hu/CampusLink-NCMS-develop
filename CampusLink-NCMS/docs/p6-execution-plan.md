...
P0-P5 已完成并通过 Windows 实机联调验证：教师端可启动并自动初始化 SQLite 数据库，学生端可完成设备注册并建立 WebSocket 心跳连接。P6 开发已完成全部核心任务，教师端 API + 学生端模块 + 前端页面均已实现并通过编译。

## 当前完成状态（2026-06-15）

P6 分支 `cb82479` 已提交，28 个文件变更，+3905 行代码。

已完成：
- 数据库迁移 0006-0008（attendance_records, inspection_records, photos）
- 教师端 10 个 API 路由（签到、检查、告警、图片上传）
- 学生端 3 个模块（attendance, inspection, student_auth）
- 前端 2 个页面（Attendance.vue, Alerts.vue）+ 3 个 API 模块

待完成：
- 实机联调验证（Windows 端到端测试）
- 图片压缩功能
- 签到记录导出
...
> 学生机开机后沿用上次关机前的模式；未配置过时初始化为开放模式。
> 是否签到及签到严格程度取决于设备当前所处模式：

| 当前模式 | 是否需要签到 | 签到方式 |
|----------|--------------|----------|
| 开放模式（open） | 需要 | 宽松签到：弹窗输入使用者姓名即可，姓名记入备注 |
| 授课模式（teaching） | 需要 | 严格签到：弹窗验证学号+密码，必须为学生库中的学生 |
| 考试模式（exam） | 不需要 | 无签到动作 |
| 锁定模式（locked） | 不需要 | 无签到动作，设备完全锁定，仅教师端远程解锁或超级密码解锁 |

> 注：原"条件开放模式"（conditional_open）已从产品设计中移除，不再作为模式存在。
